...
class Model:
	...
	def __init__(self, char_list, decoder_type=DecoderType.BestPath, must_restore=False, dump=False, ru=False):
	...
	def infer_batch(self, batch, calc_probability=False, probability_of_gt=False):
		...
		numBatchElements = len(batch.imgs)
		evalRnnOutput = self.dump or calc_probability
		...
		decoded = evalRes[0]
		texts = self.decoder_output_to_text(decoded, numBatchElements)
	...
	def save(self):
		"""save model to file"""
		self.snapID += 1
		self.saver.save(self.sess, 'model_ru/snapshot', global_step=self.snapID)

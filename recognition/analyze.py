...
def analyzePixelRelevance():
	...
	(_, probs) = model.infer_batch(batch, calcProbability=True, probabilityOfGT=True)
	origProb = probs[0]
...
def analyzeTranslationInvariance():
	...
	# compute probabilities
	(texts, probs) = model.infer_batch(batch, calcProbability=True, probabilityOfGT=True)
